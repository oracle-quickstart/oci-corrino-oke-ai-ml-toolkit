...
| recipe_mode                                  | string  | Yes      | One of the following: `service`, `job`, `shared_node_pool` or `raycluster`. Enter `service` for inference recipe deployments, `job` for fine-tuning recipe deployments, `shared_node_pool` for creating a shared node pool and `raycluster` when doing multinode inference deployments.                                                                                   |
