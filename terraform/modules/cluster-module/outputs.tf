...
output "oke_vcn_ocid" {
  ...
  depends_on = [oci_core_virtual_network.oke_vcn]
}

output "oke_k8s_endpoint_subnet_ocid" {
  value = oci_core_subnet.oke_k8s_endpoint_subnet[0].id

  depends_on = [oci_core_subnet.oke_k8s_endpoint_subnet]
}

output "oke_nodes_subnet_ocid" {
  value = oci_core_subnet.oke_nodes_subnet[0].id

  depends_on = [oci_core_subnet.oke_nodes_subnet]
}

output "oke_lb_subnet_ocid" {
  value = oci_core_subnet.oke_lb_subnet[0].id

  depends_on = [oci_core_subnet.oke_lb_subnet]
}
