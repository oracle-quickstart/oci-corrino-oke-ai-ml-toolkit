# OCI AI Blueprints
OCI AI Blueprints is a platform for deploying, scaling, and monitoring AI workloads, reducing your GPU onboarding time from weeks to minutes. It builds on top of Kubernetes and plumbs popular open-source projects like KEDA (scale controller), Multi-Instance GPUs, and Ray Serve (distributed model serving) in a single API scheduler.

The key features are:
1. **AI Blueprints for Custom and Open Models/Datasets**: OCI-verified, pre-packaged blueprints to deploy containerized AI workloads in minutes. Built for scenarios such as LLM Inference and Fine-Tuning.
2. **Infra Automation**: Automatic provisioning of infrastructure components such as GPU nodes and load balancers for deploying AI workloads.
3. **Auto-Scaling with KEDA**: Auto-scaling based on application metrics such as inference latency and throughput with a few JSON edits.
4. **GPU Sharding / Multi-Intance GPUs (MIG)**: Enable MIG with a simple switch when deploying workloads.
5. **Distributed LLM inference**: Deploy LLMs in a distributed manner across multiple nodes.
6. **Single Pane of Glass**: Portal for an intuitive UX to deploy workloads and track them.
7. **API**: API for integration with your internal apps and development platforms.
8. **Easy Observability and Monitoring**: GPU and workload observability with open-source solutions like Prometheus, Grafana, and MLFlow, which come pre-integrated.
...
1. [Getting Started](#getting-started)
    ...
    - [Step 3: Install and Access OCI AI Blueprints](#step-3-install-and-access-oci-ai-blueprints)
    - [Step 4: Deploy a vLLM Inference blueprint](#step-4-deploy-a-vllm-inference-blueprint)
    - [Step 5: Undeploy the blueprint](#step-5-undeploy-the-blueprint)
    - [Additional Resources](#additional-resources)
2. [About OCI AI Blueprints](#about-oci-ai-blueprints)
    - [Problem](#problem)
    - [What is OCI AI Blueprints and how does it help?](#what-is-oci-ai-blueprints-and-how-does-it-help)
    - [What are Blueprints?](#what-are-blueprints)
    - [Blueprint Configuration Options](#blueprint-configuration-options)
    - [What is in this repo?](#what-is-in-this-repo)
3. [Features](#features)
4. [Ways to Access OCI AI Blueprints](#ways-to-access-oci-ai-blueprints)
...
In this "Getting Started" guide, we will walk you through 5 steps:

1. Setting up the required policies in your tenancy
2. Creating an OKE cluster
3. Installing OCI AI Blueprints in your tenancy and accessing OCI AI Blueprints's UI/API
4. Deploying and monitoring a OCI AI Blueprints blueprint
5. Undeploying a blueprint
...
1. You must have an OKE cluster in your tenancy with the following configuration ([instructions for creating a new OKE cluster](https://docs.oracle.com/en-us/iaas/Content/ContEng/Tasks/create-cluster.htm))
...
5. After the Job status changes to `Succeeded`, go to the Application Information tab under Stack Details in the OCI Console. Click on “Corrino API URL” button to access the OCI AI Blueprints API. Click on "Corrino Portal URL" to access the OCI AI Blueprints Portal.

### Step 4: Deploy a vLLM Inference blueprint

1. Go to `<your-corrino-api-url>/deployment` from a web browser (you can find the OCI AI Blueprints API URL in the Application Information tab under Stack Details. See Step 3(5) above.)
2. Copy and paste this [sample inference blueprint](docs/sample_recipes/vllm_inference_sample_recipe.json) in the “Content:” text area and click “POST”
...
Undeploy the blueprint to free up the GPU again by going to the `<your-corrino-api-url>/undeploy/<deployment_id>` in your web browswer and sending the following POST request:
...
OCI AI Blueprints Sample Blueprints: [Sample Blueprints](docs/sample_recipes/README.md)
...
6. And more! Read more about blueprint configuration [here](docs/api_documentation/README.md) and see sample blueprint configurations [here](docs/sample_recipes/README.md).
...
5. OCI AI Blueprints’s front-end and back-end containers deployed to an OKE cluster of your choice
...
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| Customizing Blueprints  | Existing blueprints provided by the OCI AI Blueprints team can be customized to fit your specific AI workload needs                                                                                                                                                                                                    | [Documentation](docs/customizing_recipes/README.md) |
...
OCI AI Blueprints’s installation Terraform deploys the following:

1. OCI AI Blueprints’s front-end and back-end containers
2. Grafana and Prometheus for infrastructure monitoring
3. MLFlow for tracking experiment-level metrics
4. KEDA for dynamic auto-scaling based on AI/ML workload metrics rather than infrastructure metrics
...
All blueprints available are available [here](docs/sample_recipes/README.md). Blueprints are customizable to fit your needs, view [blueprint configuration info](docs/api_documentation/README.md) for more information on blueprint configuration. If you are interested in additional blueprints, please contact us.
